...
class Model(object):
    def __init__(self, vsize, ctx='cpu', path=None):
        self.ctx = mx.gpu() if ctx == 'gpu' else mx.cpu()

        if path == None:
            self.W1 = nd.random_uniform(-.01, .01, shape=(2048, 1024), ctx=self.ctx)
            ...
            self.emb = nd.random_uniform(-1, 1, shape=(vsize, 1024), ctx=self.ctx)
        else:
            self.W1, self.b1, self.Wy, self.by, self.emb = nd.load(path)

        self.params = [self.W1, self.b1, self.Wy, self.by, self.emb]
    ...
    def save(self, path):
        return nd.save(path, self.params)
